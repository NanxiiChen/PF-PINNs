...
class GeoTimeSampler:
    ...
    def bc_sample(self, bc_num: int, strategy: str = "lhs", xspan=[-0.025, 0.025]):
        ...
        xyts = np.vstack([xyts_left, xyts_right, xyts_top, top, left, right])
    ...
    def ic_sample(self, ic_num, strategy: str = "lhs", local_area=[[-0.1, 0.1], [0, 0.1]]):
        ...
        xys_local_1 = pc.make_lhs_sampling_data(mins=[-0.3, 0],
                                                maxs=[0.3, 0.15],
                                                num=ic_num*4)
        xys_local_2 = pc.make_lhs_sampling_data(mins=[-0.15, 0.35],
                                                maxs=[0.15, 0.50],
                                                num=ic_num*4)
        xys = np.vstack([xys, xys_local_1, xys_local_2])
